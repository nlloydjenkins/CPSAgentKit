...
function buildProjectStateSection(state: ProjectState): string {
  ...
  const lines: string[] = [
    "## Current Agent Workbench Project State",
    "",
    "This section describes the developer's installed and initialised Agent Workbench workspace, not the Agent Workbench tool source repository.",
    "",
    `- **Current phase:** ${phase}`,
    ...
    `- **Best practices:** ${state.hasBestPractices ? ".agent-workbench/bestpractices/ exists" : "no synced best practices"}`,
  ];
...
export async function generateInstructions(
  ...
  const knowledgeFileList = knowledgeFiles.map(
    ({ filename }) => `- \`.agent-workbench/knowledge/${filename}\``,
  );
  ...
  const bestPracticesFileList = bestPracticesFiles.map(
    ({ filename }) => `- \`.agent-workbench/bestpractices/${filename}\``,
  );
  ...
  const templateDirList = templateDirNames.map(
    (name) => `- \`.agent-workbench/templates/${name}/\``,
  );
  ...
  const parts = [
    "<!-- AUTO-GENERATED by Agent Workbench for this Copilot Studio workspace. Do not edit manually. Regenerated on every knowledge sync. -->",
    "",
    template,
  ];
