...
export function parseGitHubUrl(url: string): { owner: string; repo: string } {
	...
	if (!match) {
		throw new Error(`Invalid GitHub repo URL: ${url}. Expected format: https://github.com/owner/repo`);
	}
...
function httpsGet(url: string): Promise<string> {
	return new Promise((resolve, reject) => {
		...
		if (!allowedHosts.includes(parsed.hostname)) {
			reject(new Error(`Blocked request to disallowed host: ${parsed.hostname}`));
			return;
		}
		...
		https.get(options, (res) => {
			...
			if (res.statusCode === 301 || res.statusCode === 302) {
				...
				if (!allowedHosts.includes(redirectUrl.hostname)) {
					reject(new Error(`Blocked redirect to disallowed host: ${redirectUrl.hostname}`));
					return;
				}
			...
			if (res.statusCode === 403) {
				reject(new Error('GitHub API rate limit exceeded. Try again later or configure an access token.'));
				return;
			}
			if (res.statusCode === 404) {
				reject(new Error('Knowledge repo or path not found. Check cpsAgentKit.knowledgeRepoUrl and the knowledge path in config.json.'));
				return;
			}
