...
function defaults(): CpsConfig {
	return {
		knowledgeRepoUrl: getSettingOrDefault('knowledgeRepoUrl', 'https://github.com/nlloydjenkins/CPSAgentKit'),
...
export async function writeConfig(workspaceRoot: string, config: CpsConfig): Promise<void> {
	const dir = configDirPath(workspaceRoot);
	await fs.mkdir(dir, { recursive: true });
	await fs.writeFile(configPath(workspaceRoot), JSON.stringify(config, null, 2), 'utf-8');
}
...
export function getEffectiveRepoUrl(config: CpsConfig): string {
	...
	return getSettingOrDefault('knowledgeRepoUrl', 'https://github.com/nlloydjenkins/CPSAgentKit');
}
