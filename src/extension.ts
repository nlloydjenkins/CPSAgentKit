...
export function activate(context: vscode.ExtensionContext): void {
	...
	autoSyncOnOpen(extensionPath).catch((err) => {
		...
		vscode.window.showWarningMessage(`CPSAgentKit: Auto-sync failed: ${message}`);
	});
}
...
async function autoSyncOnOpen(extensionPath: string): Promise<void> {
	const syncOnOpen = vscode.workspace.getConfiguration('cpsAgentKit').get<boolean>('syncOnOpen', true);
	if (!syncOnOpen) {
		return;
	}
