...
async function getMultiLineInput(prompt: string, placeholder: string): Promise<string | undefined> {
...
export async function createSpecCommand(): Promise<void> {
	...
	// Step 3: Capabilities
	vscode.window.showInformationMessage('Now list what the agent should do. Add items one at a time. Press Escape when done.');
	const capabilities = await collectList(
		'What should the agent do?',
		'e.g. Search Dataverse tables by name or keyword',
	);
	if (capabilities.length === 0) {
		vscode.window.showWarningMessage('CPSAgentKit: At least one capability is required.');
		return;
	}

	// Step 4: Exclusions
	vscode.window.showInformationMessage('Now list what the agent should NOT do. Press Escape when done.');
	...
	vscode.window.showInformationMessage('Describe what success looks like — concrete examples. Press Escape when done.');
	...
	vscode.window.showInformationMessage('List the knowledge sources or systems the agent needs. Press Escape when done.');
	...
	const interactionStyle = await vscode.window.showQuickPick(
		[
			{ label: 'Free-form chat', description: 'User asks anything, agent responds naturally', detail: 'free-form' },
			{ label: 'Menu/command-driven', description: 'Agent presents options, user picks from a menu', detail: 'menu' },
			{ label: 'Hybrid', description: 'Menu for main flows, free-form for help/questions', detail: 'hybrid' },
		],
		{
	...
	// Generate spec.md
	const content = buildSpec(agentName, purpose, capabilities, exclusions, successExamples, knowledgeSources, interactionModel);
	...
	vscode.window.showInformationMessage(`CPSAgentKit: spec.md created for "${agentName}". Review and refine with Copilot.`);
}
