...
export async function buildAgentCommand(): Promise<void> {
	...
	const scope = await vscode.window.showQuickPick(
		[
			{ label: 'Full build', description: 'Generate all agent config: instructions, topics, tool descriptions', detail: 'full' },
			{ label: 'Agent instructions only', description: 'Generate/update agent instructions for the Overview page', detail: 'instructions' },
			{ label: 'Topic descriptions only', description: 'Generate/update topic trigger descriptions', detail: 'topics' },
			{ label: 'Tool descriptions only', description: 'Generate/update tool/action descriptions', detail: 'tools' },
			{ label: 'Rebuild from test feedback', description: 'Paste test output and get specific fixes', detail: 'test' },
		],
		{
	...
	if (scope.detail === 'test') {
		testOutput = await vscode.window.showInputBox({
	...
	// Build the prompt
	const prompt = composeBuildPrompt(scope.detail!, spec, architecture, agentYaml, testOutput);
...
function composeBuildPrompt(
	...
	const agentContext = agentFolders.length > 0
