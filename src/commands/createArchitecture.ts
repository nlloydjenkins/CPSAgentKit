...
export async function createArchitectureFromDocs(root: string): Promise<void> {
  ...
  sections.push(
    ...
    "- Prefer a shared Dataverse CRUD scaffold (one read/list, one write, one delete per agent) rather than creating connector actions per table or per business function unless the data model genuinely requires specialization",
    ...
    "- In Applied CPS Constraints, record the specific platform constraints that materially shaped the architecture (e.g. why single-agent vs multi-agent, why parent owns MCP, why prompt tool instead of child agent)",
    "- In Best-Practice Decisions, record the repo-guided choices made (e.g. shared CRUD scaffold, tool-first instructions, knowledge source scoping)",
    "- In Known Risks, record any remaining CPS risks or deferred exceptions that need review before build (e.g. flow identity, content moderation level, missing knowledge source descriptions)",
    "- Check for and rewrite these anti-patterns: mega-agent designs exceeding 25-30 tools, child agents relying on MCP through parent orchestration, Power Automate flows where connector actions are sufficient, overlapping tool/agent scopes, over-granular Dataverse actions",
