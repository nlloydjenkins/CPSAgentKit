...
async function collectList(prompt: string, placeholder: string): Promise<string[]> {
	const items: string[] = [];
	let i = 1;
	while (true) {
...
async function collectAgent(agentNumber: number): Promise<AgentEntry | undefined> {
	...
	const typePick = await vscode.window.showQuickPick(
		[
			{ label: 'Standalone', description: 'Single agent, no parent/child relationship', detail: 'standalone' },
			{ label: 'Parent (router)', description: 'Orchestrates child/connected agents', detail: 'parent' },
			{ label: 'Child agent', description: 'Embedded in parent, shares environment', detail: 'child' },
			{ label: 'Connected agent', description: 'Independent, published separately', detail: 'connected' },
		],
		{
	...
	vscode.window.showInformationMessage(`List the tools/actions "${name}" uses. Escape when done.`);
	const tools = await collectList(
		`Tools for "${name}"`,
		'e.g. MCP Dataverse Server, List Records action',
	);

	vscode.window.showInformationMessage(`List knowledge sources for "${name}". Escape when done.`);
	...
	const instructions = await vscode.window.showInputBox({
		...
		placeHolder: 'e.g. Always use the MCP tool before general knowledge. Read-only, decline writes.',
		ignoreFocusOut: true,
	}) || '';

	return { name, role, type: typePick.detail || 'standalone', tools, knowledge, instructions };
}
...
function buildArchitecture(
	...
	lines.push('## Routing Logic', '', routingLogic || 'Single agent — no routing needed.', '');
	...
	for (const tool of tools) {
		lines.push(`| ${tool.name} | ${tool.ownerAgent} | ${tool.purpose} | ${tool.manualStep ? 'Yes' : 'No'} |`);
	}
	if (tools.length === 0) {
...
export async function createArchitectureCommand(): Promise<void> {
	...
	const overview = await vscode.window.showInputBox({
		...
		placeHolder: 'e.g. Single agent that uses MCP Dataverse Server to help users explore tables and data',
		ignoreFocusOut: true,
	});
	if (!overview) {
		return;
	...
	const agentCountPick = await vscode.window.showQuickPick(
		['1 — Single agent', '2 — Two agents', '3 — Three agents', '4+ — More (specify)'],
		{
	...
	let routingLogic = '';
	if (agents.length > 1) {
		routingLogic = await vscode.window.showInputBox({
			title: 'CPSAgentKit: Architecture (4/5)',
			prompt: 'How does the parent route between agents?',
			placeHolder: 'e.g. Routes billing questions to Billing Agent, tech support to Tech Agent',
			ignoreFocusOut: true,
		}) || '';
	}
	...
	for (const agent of agents) {
		for (const toolName of agent.tools) {
			const purpose = await vscode.window.showInputBox({
				...
				placeHolder: 'e.g. Queries Dataverse tables, columns, and row data via MCP protocol',
				ignoreFocusOut: true,
			}) || toolName;
	...
	vscode.window.showInformationMessage('List any manual portal steps needed. Escape when done.');
	...
	const content = buildArchitecture(overview, agents, routingLogic, tools, manualSteps);
	await fs.writeFile(archPath, content, 'utf-8');

	const doc = await vscode.workspace.openTextDocument(archPath);
	await vscode.window.showTextDocument(doc);

	vscode.window.showInformationMessage('CPSAgentKit: architecture.md created. Review and refine with Copilot.');
}
