...
async function parseBuildState(archPath: string): Promise<Array<{ label: string; done: boolean; line: number }>> {
...
async function updateBuildState(archPath: string, lineNumber: number, done: boolean): Promise<void> {
	const content = await fs.readFile(archPath, 'utf-8');
	const lines = content.split('\n');
...
export async function buildCommand(): Promise<void> {
	...
	if (items.length === 0) {
		vscode.window.showWarningMessage('CPSAgentKit: No build state checklist found in architecture.md.');
		return;
	}
	...
	if (item.done) {
		// Offer to uncheck
		const undo = await vscode.window.showQuickPick(['Mark as not done', 'Cancel'], {
			placeHolder: `"${item.label}" is complete. Undo?`,
		});
		if (undo === 'Mark as not done') {
			await updateBuildState(archPath, item.line, false);
			vscode.window.showInformationMessage(`CPSAgentKit: "${item.label}" marked as not done.`);
		}
		return;
	}
	...
	switch (action.detail) {
		case 'mark':
			...
			vscode.window.showInformationMessage(`CPSAgentKit: "${item.label}" marked as done.`);
			break;
		case 'help': {
			// Build a Copilot-ready prompt for this step and copy to clipboard
			const prompt = buildGuidancePrompt(item.label);
			await vscode.env.clipboard.writeText(prompt);
			vscode.window.showInformationMessage(
				`CPSAgentKit: Guidance prompt for "${item.label}" copied to clipboard. Paste into Copilot Chat.`,
			);
			break;
		}
		case 'open': {
...
function buildGuidancePrompt(stepLabel: string): string {
	const prompts: Record<string, string> = {
		...
		'Agent instructions written': [
			'Read architecture.md and spec.md. For each agent, generate the agent instructions that should be pasted into the CPS portal Overview page.',
	...
	return prompts[stepLabel] || `Help me complete this build step: "${stepLabel}". Read architecture.md and spec.md for context.`;
}
