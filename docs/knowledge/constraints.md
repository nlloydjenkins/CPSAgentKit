...
- Connectors, MCP servers, workflows, and connection references: reference-backed first when a verified export/API pattern exists and tenant-specific connection/auth values are available, including real connection reference logical names from the active workspace. Portal-first is the fallback when no verified path or binding exists; then sync and edit the generated files locally.
...
- **Portal workflow fallback:** Create each new action in the CPS portal (Tools → Add a tool → duplicate + pre-target), then sync locally via Get Changes when no verified export/API path exists. Edit only YAML-safe fields such as `modelDisplayName` and `modelDescription` locally after sync.
- **Reference-backed exception:** Manual `TaskDialog` action scaffolds can be attempted with explicit developer opt-in, a working reference export, or a product-validated reference build for the exact first-party pattern only when the active workspace contains real tenant connection reference logical names. They must include a root `connectionreferences.mcs.yml`, valid `action.connectionReference` values, portal/export-style operation IDs such as `InvokeMCP`, `MyProfile_V2`, `PostMessageToConversation`, or `SendEmailV2` only when verified, Apply Changes acceptance, Get Changes round-trip verification, and Activity Map runtime testing before they count as complete. If the root manifest or exported bindings are missing, do not create placeholder action YAML.
- MCP subtools are portal/runtime-discovered and may not be exported into YAML. Do not hand-author `knownTools` or mutate `action.operationDetails`; if subtools are missing, use the four-step Save remediation (disable tool + Save, disable subtools + Save, enable tool + Save, refresh tools + Save) and validate runtime execution in Activity Map.
- Topic YAML scaffolds are safe for routing, questions, confirmation, and messaging when they match exported shapes. Do not hand-author topic-owned MCP or connector execution nodes without a verified portal-generated pattern from the target environment.
