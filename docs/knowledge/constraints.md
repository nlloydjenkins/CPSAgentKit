...
- Connectors, MCP servers, workflows, and connection references: reference-backed first when a verified export/API pattern exists and tenant-specific connection/auth values are available. Portal-first is the fallback when no verified path exists; then sync and edit the generated files locally.
...
- **Reference-backed exception:** Manual `TaskDialog` action scaffolds can be attempted with explicit developer opt-in, a working reference export, or a product-validated reference build for the exact first-party pattern. They must include a root `connectionreferences.mcs.yml`, valid `action.connectionReference` values, portal/export-style operation IDs such as `InvokeMCP`, `MyProfile_V2`, `PostMessageToConversation`, or `SendEmailV2` only when verified, Apply Changes acceptance, Get Changes round-trip verification, and Activity Map runtime testing before they count as complete.
- MCP subtools are portal/runtime-discovered and may not be exported into YAML. Do not hand-author `knownTools` or mutate `action.operationDetails`; if subtools are missing, use the portal off-refresh-on remediation and validate runtime execution in Activity Map.
