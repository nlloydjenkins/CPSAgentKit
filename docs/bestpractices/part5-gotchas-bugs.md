...
Power Automate flows invoked as agent actions run with the **flow author's (maker's) credentials by default** — not the end user's. Every connection in the flow uses the maker's identity unless explicitly reconfigured to "Provided by run-only user." More precisely, a flow or action executes under its _configured connection/runtime context_, which may be maker-owned, connection-owner based, run-only-user based, or otherwise configured depending on how each connection and connector are set up. This means the agent accesses data with that configured identity's permissions regardless of who is chatting. Audit trails attribute actions to the maker, not the actual user. If the maker leaves the organisation or their account is disabled, all flows using their embedded credentials break silently. Use a dedicated service account for production flows and configure per-connection delegation where possible. **Do not assume the signed-in Teams user is the authorization boundary** — validate the actual connection identity and enforce user entitlement in your Dataverse/API logic rather than relying on the chatting user's identity to scope data access.
