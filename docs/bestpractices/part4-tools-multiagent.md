...
- **Don't treat the chatting user as the authorization boundary.** A flow or action executes under its _configured connection/runtime context_ — which may be maker-owned (embedded), connection-owner based, run-only-user (delegated), or otherwise configured depending on how each connection and the connector are set up. The signed-in Teams user is **not** automatically the security boundary. Do not assume the platform restricts data to what the chatting user can access. Validate the actual connection identity and enforce per-user entitlement in your Dataverse/API logic (e.g. filter records by the verified caller, check role membership) rather than relying on the connection's identity to scope access.
